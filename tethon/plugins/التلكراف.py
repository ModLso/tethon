...
@tethon.ar_cmd(pattern="(ت(ل)?ك(راف)?) ?(m|t|ميديا|نص)(?:\s|$)([\s\S]*)")
async def _(event):
    tethonevent = await edit_or_reply(event, "** ⌔∮ جار انشاء رابط تلكراف ثواني**")
    if BOTLOG:
    ...
    if input_str in ["ميديا", "m"]:
        ...
        await tethonevent.edit(f"**⌔∮ تم التحميل الى {downloaded_file_name}**")
        if downloaded_file_name.endswith((".webp")):
            resize_image(downloaded_file_name)
        try:
            media_urls = upload_file(downloaded_file_name)
        except exceptions.TelegraphException as exc:
            await tethonevent.edit(f"** ⌔∮ خطأ : **\n**{exc}**")
            os.remove(downloaded_file_name)
        else:
            end = datetime.now()
            ms = (end - start).seconds
            os.remove(downloaded_file_name)
            await tethonevent.edit(
        ...
        ms = (end - start).seconds
        tethon = f"https://telegra.ph/{response['path']}"
        await tethonevent.edit(
            f"**الرابط**:  [اضغط هنا]({tethon})\
                 \n**الوقت المستغرق** : {ms} **انتضر**",
            link_preview=False,
        )
